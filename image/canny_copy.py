...
def noise_dataset(input_dir):
    ...
    dataset_list = []
    for cls in dataset: #数据集中的每个文件夹
        image_path_list = []
        for image_path in cls.image_paths:
            image_path_list.append(image_path)
        dataset_list.append([cls.name,image_path_list]) #cls是[姓名，[图片1路径，图片2路径。。。]]
    with Pool(10) as p:
        muti_result = p.starmap(mutiprocess,tqdm.tqdm(dataset_list,total=len(dataset_list)))

    
def mutiprocess(name,path_list): 
    output_dir = "../facenet/understand_facenet-master/understand_facenet/data/lfw_lap"
...
if __name__ == '__main__':
    ...
    # #file1.close()
    # input_path = '../facenet/understand_facenet-master/understand_facenet/data/lfw_funneled_160'
