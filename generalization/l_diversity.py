...
def diversity(data, partition, column):
    return len(data[column][partition].unique())


def is_l_diversity(data, partition, sensitive, l=2):
    return diversity(data, partition, sensitive) >= l
...
if __name__ == "__main__":
    ...
    finished_l_diversity_partitions = partition_dataset(data, quasi_id, sensitive, full_spans,
                                                        lambda *args: is_k_anonymity(*args) and is_l_diversity(*args))

    data_diversity = build_anonymized_dataset(data, finished_l_diversity_partitions, quasi_id, sensitive)
    data_diversity.sort_values([column_x, column_y, sensitive])
    data_diversity.to_csv(sys.argv[2], index=False)
    print("===============l-diversity===============")
