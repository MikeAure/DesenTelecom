...
file_names = [f for f in os.listdir(directory_path) if
              os.path.isfile(os.path.join(directory_path, f)) and not f.startswith(prefix_to_exclude)]
# file_names = [f for f in os.listdir(directory_path) if not f.startswith(prefix_to_exclude)]

# 可选：仅获取文件，而非目录
file_paths = [os.path.join(directory_path, filename) for filename in file_names if
