    ...
    <script type="text/javascript">
        ...
        window.onload = function () {
            document.getElementById("exportBtn").addEventListener('click', function () {
                fetch("/File/recvOnLineTaxiFile")
                    .then(response => {
                        // 获取Content-Disposition头部
                        const contentDisposition = response.headers.get('Content-Disposition');
                        // 使用正则表达式从Content-Disposition头部中提取文件名
                        const match = contentDisposition && contentDisposition.match(/filename\*=UTF-8''(.+)/);
                        let fileNameRec = match ? decodeURIComponent(match[1]) : "default_filename.txt";

                        // 如果Content-Disposition使用较简单的格式设置文件名，可以使用以下方式
                        if (!match) {
                            const matchSimple = contentDisposition && contentDisposition.match(/filename="?(.+)"?/);
                            ...
                            if ("xlsx" === fileExtension) {
                                ...
                                fetch("/onlinetaxi2param/list")
                                    ...
                                    .then(data => {
                                        ...
                                        document.getElementById("showTemplate").click();
                                        document.getElementById("setTemplate").click();
                                        console.log("sheet:" + sheet)
                                        //提交脱敏参数，请求脱敏
                                        console.log("formData: " + formData);
                                        // formData.append("params", JSON.stringify(data));
                                        //     .then(response => response.json())
                                        //     .then(data => formData.append("params", data));
            ...
            //     data.forEach(item => {
            //         if (!columnNames[item.attributeName]) {
            //             columnNames[item.attributeName] = [];
            //         }
            //         columnNames[item.attributeName].push(item.infoContent);
            //     });
            //
            //     // 转换为适合SheetJS的格式
            //     let result = [];
            //     let maxLen = 0;
            //     // 计算最大长度以确定行数
            //     Object.values(columnNames).forEach(arr => maxLen = Math.max(maxLen, arr.length));
            //
            //     for (let i = 0; i < maxLen; i++) {
            //         let row = {};
            //         for (const [attributeName, contents] of Object.entries(columnNames)) {
            //             row[attributeName] = contents[i] || ''; // 使用空字符串填充缺失的值
            //         }
            //         result.push(row);
            //     }
            //
